...
export const getLibrary = createServerFn({ method: "GET" }).handler(async () => {
  const [libRaw, settingsRaw, skillsRaw] = await Promise.all([
    getValues("Exercise%20Library!A5:H200"),
    getValues("Settings!A14:K60"),
    getValues("Skills%20Tracker!B10:C30"),
  ]);
  ...
  for (const r of skillsRaw) {
    ...
    const focusArea = r[1] ?? "";
    exercises.push({
      workoutType: focusArea === GRIP_WORKOUT_TYPE ? GRIP_WORKOUT_TYPE : SKILL_WORKOUT_TYPE,
      focusArea,
      name,
      equipment: "Bodyweight",
      metric: "Skill practice",
      suggestedSets: "",
      suggestedReps: "",
      notes: "Legacy skill from Skills Tracker",
    });
  }
  for (const name of GRIP_FALLBACKS) {
    if (names.has(name.toLowerCase())) continue;
    names.add(name.toLowerCase());
    exercises.push({
      workoutType: GRIP_WORKOUT_TYPE,
      focusArea: "",
      name,
      equipment: "Grip tool",
      metric: "Hold / load",
  ...
  for (let i = 1; i < settingsRaw.length; i++) {
    ...
    if (row[0]) {
      ...
      if (workoutType !== "Recovery" && !workoutTypes.includes(workoutType)) {
    ...
    if (row[9]) assistanceTypes.push(row[9]);
    if (row[10]) qualities.push(row[10]);
  }

  if (!workoutTypes.includes(SKILL_WORKOUT_TYPE)) workoutTypes.push(SKILL_WORKOUT_TYPE);
  if (!workoutTypes.includes(GRIP_WORKOUT_TYPE)) workoutTypes.push(GRIP_WORKOUT_TYPE);
  ...
  return {
    ...
    climbingTypes,
    trackingModes,
    assistanceTypes: assistanceTypes.length
      ? assistanceTypes
      : ["None", "Band", "Counterweight", "Partner", "Wall/support", "Other"],
    qualities: qualities.length ? qualities : ["Poor", "Okay", "Good", "Great"],
  };
});

export const getRecentLogs = createServerFn({ method: "GET" }).handler(async () => {
  const rows = await getValues("Workout%20Log!A5:X1000");
  const populated = rows.filter((r) => r[4]); // Exercise col
  const recent = populated.slice(-15).reverse().map((r) => ({
    date: r[0] ?? "",
    workoutType: normalizeWorkoutType(r[2] ?? ""),
    focusArea: r[3] ?? "",
    exercise: r[4] ?? "",
    sets: r[5] ?? "",
    reps: r[6] ?? "",
    ...
    notes: r[15] ?? "",
    entryKind: r[18] ?? "",
    progressionLevel: r[19] ?? "",
    holdSeconds: r[20] ?? "",
    assistanceType: r[21] ?? "",
    assistanceDetail: r[22] ?? "",
    quality: r[23] ?? "",
  }));
  return { recent };
});
...
const WorkoutInput = z.object({
  ...
  notes: longText(2000),
  progressionLevel: shortText(200),
  holdSeconds: shortText(20),
  assistanceType: shortText(100),
  assistanceDetail: shortText(200),
...
export const addWorkout = createServerFn({ method: "POST" }).middleware([appSecretAuth])
  ...
  .handler(async ({ data }) => {
    ...
    // Write A, C:M, P, and appended skill metadata in S:X.
