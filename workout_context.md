...
Climbing is no longer a separate top-level Log tab. In the main Log form, choosing `Climbing` changes movement options to `Bouldering Session`, `Ropes/Belay`, `Kilter`, and `Mix`, and shows climbing-specific fields such as hours, boulders/routes, max grade, intensity, and Kilter-only gradient.
...
- Other: `Other Session`
- Mobility/Flexibility positions: `Side Split`, `Pancake`, `Pike`, `Bridge`, `Shoulder Flexion`, using Distance (cm), Hold (sec), and Feel (1-5)
- Climbing: `Bouldering Session`, `Ropes/Belay`, `Kilter`, `Mix`
